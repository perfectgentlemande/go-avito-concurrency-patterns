...
func main() {
	...
	producers = append(producers, producer("Charlie", done, &wg))

	fanIn1 := make(chan payload)
	fanIn2 := make(chan payload)

	wg.Add(3)
	consumer(producers, done, &wg, fanIn1)

	wg.Add(3)
	consumer(producers, done, &wg, fanIn2)
	...
	go func() {
		...
		for {
			...
			case <-done:
				return
			case v := <-fanIn1:
				fmt.Println("fanIn1 got:", v)
			case v := <-fanIn2:
				fmt.Println("fanIn2 got:", v)
			}
		}
	}()
